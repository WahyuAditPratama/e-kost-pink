...
class Userbank extends BaseController
{
	...
	function __construct()
	{
		...
		$this->models 	= new UserbankModel($this->request);
	}
	function index()
	{
		$data["title"] 	= "Userbank";
		...
		if ($this->request->getPost()) {
			...
			if (!$this->validate($validasi)) {
				...
				if (!empty($_FILES['gambar']['name'])) {
					...
					$imagex->resize(128, 128, true, 'height');
	...
	function load()
	{
		...
		foreach ($list as $rows) {
			...
			$row[] = $pathfile;
			$row[] = ($rows->status) ? '<span class="badge bg-success">Aktif</span>' : '<span class="badge bg-danger">Nonaktif</span>';
			$row[] = $button . '
